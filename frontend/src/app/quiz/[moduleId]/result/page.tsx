...
import { CheckCircle2, XCircle, ChevronLeft, Trophy, ArrowRight, BookOpen, Target, Sparkles, Clock, RefreshCcw } from 'lucide-react';
...
export default function QuizResultPage() {
  ...
  const perf = getPerformanceMessage(result.percentage !== undefined ? result.percentage : result.score);
      ...
      <div className="max-w-[800px] mx-auto">
        ...
        <section className={`${perf.bg} rounded-xl p-10 md:p-16 text-center border border-zinc-100 shadow-sm shadow-zinc-200/50 relative overflow-hidden`}>
          ...
          <div className="relative inline-block mb-10">
            <div className="text-8xl font-bold tracking-tighter text-zinc-900 ">
              {result.score}<span className="text-3xl not- text-zinc-400 ml-1">/ {totalCount}</span>
            </div>
          </div>

          <div className="max-w-xs mx-auto">
            <Progress value={(result.score / (totalCount || 1)) * 100} className="h-3 bg-white border border-zinc-100" />
          </div>
        </section>
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12">
          ...
          <div className="bg-zinc-50 rounded-3xl p-8 border border-zinc-100 flex flex-col items-center text-center">
            ...
            <p className="text-2xl font-bold text-zinc-900">{result.status === "PASSED" ? "Passed" : "Failed"}</p>
          </div>
        </section>
            ...
            <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
              {result.status !== "PASSED" && (
                <Button 
                  ...
                  className="bg-[#F26522] hover:bg-[#D54D10] text-white h-16 px-10 rounded-2xl font-bold uppercase text-xs transition-all active:scale-95 shrink-0 flex items-center justify-center gap-2"
                >
        ...
        <footer className="mt-12 text-center text-[#111] text-xs font-bold uppercase pb-10">
          &copy; 2026 ARK University LMS • Assessment Engine v3.0
        </footer>
